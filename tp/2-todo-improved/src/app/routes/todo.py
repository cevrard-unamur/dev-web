...
@login_required
def listing():
    return render_template('todo/list.html.j2', tasks = TaskDataAccess.get_tasks(current_user.get_id()))
...
def canAccessToTask(task):
    return task.user_id == current_user.get_id() or current_user.accountLevel == 2 or current_user.accountLevel == 3